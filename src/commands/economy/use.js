...
module.exports = class CMD extends Command {
    ...
    async execute({ message, args, userDB }, t) {
        ...
        if (!items.includes(args[0])) return message.reply(t("cmds:use.notAnItem"));
        const itemsThatGuyHas = items.filter(i => userDB.inventory[i] > 0);
        if (!itemsThatGuyHas.includes(args[0])) return message.reply(t("cmds:use.youDontHaveAny"));
        if (!args[1]) args[1] = 1;
        const [item, count] = args;
        if (userDB.inventory[item] >= count) return message.reply(t("cmds:use.tooMuch", {item, count}));
        ...
        switch (item) {
            case "banknote":
                ...
                break;
            default:
                throw new Error(`item, ${item} was tried to be used, but no method was found to implement`);
                break;
        }
        await userDB.save();
